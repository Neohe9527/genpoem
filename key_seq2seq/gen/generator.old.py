      ...
      for prob, cand in beams:
        ...
        for c in np.argsort(all_prob)[::-1][:beam_size]:
          ...
          if np.array([c]) not in old_cand and int(c)!=0:
              print(cand['prob_ts'] *all_prob_ts[c])
          print(cand['prob_ts'] *all_prob_ts[c]*history_alpha)
          new_cand = {
            ...
            'prob_ts' : (cand['prob_ts'] *all_prob_ts[c]) if (np.array([c]) not in old_cand and int(c)!=0) else (cand['prob_ts'] *all_prob_ts[c]*history_alpha),

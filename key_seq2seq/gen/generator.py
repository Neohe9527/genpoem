...
def get_predicted_sentence(sess, input_token_ids, vocab, model,
                           beam_size, buckets, batch_size=2,mc_search=True,debug=True,history_alpha=5):
    def model_step(enc_inp, dec_inp, dptr, target_weights, bucket_id):
      ...
      if batch_size==1:
          ...
          prob=[]
          for _logit in logits[dptr]:
              prob.append(softmax(_logit))
      #print("prob")
      #print(prob)
    ...
    #print(decoder_inputs)
    #results=[[]]*batch_size
    dec_inp=copy.deepcopy(decoder_inputs)
    form_dec_inp=(np.transpose(np.array(dec_inp, dtype =int)))
    beams, new_beams=[[]]*batch_size, [[]]*batch_size
    for i in xrange(batch_size):
        beams[i]=[(1, {'eos': 0, 'dec_inp':form_dec_inp[i], 'prob': 1})]
    for dptr in range(len(decoder_inputs)-1):
        if dptr > 0:
            target_weights[dptr] = [1.]*batch_size
            for i in xrange(batch_size):
                beams[i], new_beams[i]= new_beams[i][:beam_size], []
                beams[i]= sorted(beams[i], key=lambda x:x[0], reverse=True)

            
            train_dec_inp=np.transpose(np.array(beams))
            #print("train_dec_inp")
            #print(train_dec_inp)
            all_dec_inp=[]
            beam_all_prob_ts=[]
            for each in train_dec_inp[1]:
                dec_inp=[]
                for item in each:
                    dec_inp.append(np.array(item['dec_inp']))
                dec_inp=np.transpose(np.array(dec_inp))
                #print(dec_inp)
                all_prob_ts= model_step(encoder_inputs, dec_inp, dptr, target_weights, bucket_id)
                beam_all_prob_ts.append(all_prob_ts)
        else:
            beam_all_prob_ts=[]
            all_prob_ts=model_step(encoder_inputs, dec_inp, dptr, target_weights, bucket_id)
            beam_all_prob_ts.append(all_prob_ts)
        #beam_all_prob_ts=np.reshape(np.array(beam_all_prob_ts),(batch_size,-1,7938), order='F')
        beam_all_prob_ts=np.array(beam_all_prob_ts).transpose(1, 0, 2)
        index = 0
        #print("beam_all_prob_ts")
        #print(beam_all_prob_ts)
        for _prob_ts in beam_all_prob_ts:
            all_prob  = _prob_ts
            temp_beams=copy.deepcopy(beams[index])
            heapq.heapify(temp_beams) # since we will srot and remove something to keep N elements
            temp_new_beams=copy.deepcopy(new_beams[index])
           #beams, new_beams = new_beams[:beam_size], []
            beam_index=0
            for prob, cand in temp_beams:
               if cand['eos']:
                   #results[index] += [(prob, cand)]
                   continue
               #if debug: print("=====[beams]=====", beams[index])
           # beam search
               for c in np.argsort(all_prob[beam_index])[::-1][:beam_size]:
                   old_cand = cand['dec_inp']
                   #print("c")
                   #print(c)
                   #print("old_cand")
                   #print(old_cand)
                   #print(all_prob[beam_index][c])
                   new_cand = {
                       'eos'     : (c == data_utils.EOS_ID),
                       'dec_inp' : [(c if i == (dptr+1) else k) for i, k in enumerate(cand['dec_inp'])],
            #'prob_ts' : cand['prob_ts'] * all_prob_ts[c],
            #'prob_t'  : cand['prob_t'] * all_prob_t[c],
            #'prob'    : cand['prob'] * all_prob[c],
                       'prob' : (cand['prob'] *all_prob[beam_index][c]) if (c not in old_cand and int(c)!=0) else (cand['prob'] *all_prob[beam_index][c]*history_alpha)}
               
                   new_cand = (new_cand['prob'], new_cand) # for heapq can only sort according to list[0]
                   #print(new_cand)

                   if (len(temp_new_beams) < beam_size):
                       heapq.heappush(temp_new_beams, new_cand)
                   elif (new_cand[0] > temp_new_beams[0][0]):
                       heapq.heapreplace(temp_new_beams, new_cand)
               beam_index+=1
            new_beams[index]= temp_new_beams
            #print("new beams index")
            #print(len(new_beams[index]))
            index+=1
    results=[]
    for i in xrange(batch_size):
        #print(new_beams[i])
        results.append([cand['dec_inp']  for prob,cand in sorted(new_beams[i], key=lambda x:x[0], reverse=True)]) # flush last cands
        #print(results[i])
...
def gen_sample(sess, gen_config, model, vocab, source_inputs, source_outputs, history_alpha, mc_search=True):
    ...
    index = 0
    #print("get pre start")
    responses = get_predicted_sentence(sess, source_inputs, vocab,model, gen_config.beam_size, _buckets, gen_config.batch_size, mc_search,debug=True,history_alpha=history_alpha)
    #print("get pre end")
    #print("responses")
    #print(responses)
    form_resp=[]
    ...
    for source_query, source_answer in zip(source_inputs, source_outputs):
        ...
        #print (dec_inp)
        if mc_search==True:
            for _each in dec_inp:
                each=_each[1:len_dec_size+1]
            #print ("  (%s) -> %s" % (resp['prob'], dec_inp))
                sample_neg = source_query + each
                sample_inputs.append(sample_neg)
                sample_labels.append([1, 0])
                rep.append(each)
                rep_same.append(_each)
        else:
            sample_neg= source_query+dec_inp[:len_dec_size]
            sample_inputs.append(sample_neg)
            sample_labels.append([1, 0])

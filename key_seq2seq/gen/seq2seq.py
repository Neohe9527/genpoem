...
import gen.core_rnn as core_rnn
...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    encode_cell = cell[0]
    decode_cell = cell[1]
    encoder_cell_fw_keyword = copy.deepcopy(encode_cell)
    encoder_cell_fw_keyword = core_rnn_cell.EmbeddingWrapper(
        encoder_cell_fw_keyword,
        embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)

    #for bw
    encoder_cell_bw_keyword = copy.deepcopy(encode_cell)
    encoder_cell_bw_keyword = core_rnn_cell.EmbeddingWrapper(
            encoder_cell_bw_keyword,
            embedding_classes=num_encoder_symbols,
            embedding_size=embedding_size)
    #####birnn
    keyword_inputs = encoder_inputs[:max_len_keyword]
    #with tf.name_scope("keywords"):
    keyword_fw, keyword_bw, keyword_fw_state,keyword_bw_state = core_rnn.static_bidirectional_rnn(
                encoder_cell_fw_keyword, encoder_cell_bw_keyword,keyword_inputs, dtype=dtype,scope="keywords")
    
    print (keyword_fw)
    print(keyword_bw)
    print(keyword_fw_state)
    print(keyword_bw_state)
    ### Encoder for text
    ##for fw
    encoder_cell_fw_text = copy.deepcopy(encode_cell)
    encoder_cell_fw_text = core_rnn_cell.EmbeddingWrapper(
            encoder_cell_fw_text,
            embedding_classes=num_encoder_symbols,
            embedding_size=embedding_size)
    ##for bw
    encoder_cell_bw_text = copy.deepcopy(encode_cell)
    encoder_cell_bw_text = core_rnn_cell.EmbeddingWrapper(
            encoder_cell_bw_text,
            embedding_classes=num_encoder_symbols,
            embedding_size=embedding_size)
    #birnn
    encoder_fw = []
    encoder_fw.append(keyword_fw[-1])
    encoder_bw = []
    encoder_bw.append(keyword_bw[-1])
    text_inputs = encoder_inputs[max_len_keyword:]
    #with tf.name_scope("text"):
    text_fw,text_bw, encoder_fw_state,encoder_bw_state = core_rnn.static_bidirectional_rnn(encoder_cell_fw_text, encoder_cell_bw_text,text_inputs, dtype=dtype, scope="text")
    ### concat
    encoder_fw.extend(text_fw)
    encoder_bw.extend(text_bw)
    flat_encoder_fw = nest.flatten(encoder_fw)
    flat_encoder_bw = nest.flatten(encoder_bw)
    encoder_outputs = tuple( array_ops.concat([fw, bw], 1)
            for fw, bw in zip(flat_encoder_fw, flat_encoder_bw))
    encoder_outputs = nest.pack_sequence_as(structure=encoder_fw, flat_sequence=encoder_outputs)
    print(encoder_outputs)
    #print(encoder_fw_state)
    #print (encoder_bw_state)
    #encoder_outputs, encoder_state = core_rnn.static_rnn(
     #   encoder_cell_fw, encoder_inputs, dtype=dtype)
    if isinstance(encoder_fw_state[0], LSTMStateTuple):
        print ("LSTMStateTuple")
        encoder_state_c = (tf.concat((each1.c, each2.c), 1) for (each1,each2) in zip(encoder_fw_state,encoder_bw_state))
        encoder_state_h = (tf.concat((each1.h, each2.h), 1) for (each1,each2) in zip(encoder_fw_state,encoder_bw_state))

        #encoder_state_h = tf.concat((encoder_fw_state[0].h, encoder_bw_state.h), 1, name='bidirectional_concat_h')
        encoder_state= tuple((LSTMStateTuple(c=_c, h=_h)) for (_c, _h) in zip(encoder_state_c, encoder_state_h))
    else:
        print(encoder_fw_state)
        print(encoder_bw_state)
        temp_encoder_state = tf.concat((encoder_fw_state[0], encoder_bw_state[0]), 1)
        print (temp_encoder_state)
        encoder_state = tuple(tf.concat((each1, each2), 1) for (each1,each2) in zip(encoder_fw_state,encoder_bw_state))
    ...
    top_states = [
        array_ops.reshape(e, [-1, 1, 2*cell[0].output_size]) for e in encoder_outputs
    ]
    # use one layer to decode
    encoder_state = encoder_fw_state[-1]
    attention_states = array_ops.concat(top_states, 1)

    # Decoder.
    output_size = None
    if output_projection is None:
      cell = core_rnn_cell.OutputProjectionWrapper(decode_cell, num_decoder_symbols)

...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    with tf.variable_scope(self.scope_name):
       ...
       # pass
        ...
        decode_cell = GRUCell(size*2)
        #bw_cell = GRUCell(size)
        if use_lstm:
          print ("use lstm")
          encode_cell = BasicLSTMCell(size)
          decode_cell = BasicLSTMCell(size*2)
  ...
  def get_batch(self, train_data, bucket_id, max_keyword_len=4,type=0):
    ...
    encoder_size, decoder_size = self.buckets[bucket_id]
    text_encoder_size = encoder_size - max_keyword_len
    text_decoder_size = decoder_size - max_keyword_len

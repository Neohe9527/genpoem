...
def sentence_to_token_ids(sentence, vocabulary,
  ...
  #for each in vocabulary:
   #   print (vocabulary[each])
  pairs = sentence.strip().strip('\n').decode('utf-8').split('\t')
  source_list=[]
  dis_list=[]
  source_list.extend(pairs[0].strip().split(' '))
  dis_list.extend(pairs[1].strip().split(' '))
  #return [vocabulary.get(w, UNK_ID) for w in source_list], [vocabulary.get(w, UNK_ID) for w in dis_list]
    ...
    with gfile.GFile(data_path, mode="rb") as data_file:
      with gfile.GFile(target_path, mode="w") as tokens_file:
        ...
        for line in data_file:
          ...
          tar_token_ids, dis_token_ids = sentence_to_token_ids(line, vocabulary, tokenizer,
                                            normalize_digits)
          #print (tar_token_ids)
          tokens_file.write(" ".join([str(tok) for tok in tar_token_ids]) + "\t"+" ".join([str(tok) for tok in dis_token_ids])+'\n')
...
def prepare_chitchat_data(data_dir, vocabulary, vocabulary_size, tokenizer=None):
  ...
  train_ids_path = data_dir + ("/ids%d.train.gan" % vocabulary_size)
  ...
  data_to_token_ids(data_dir+ "/data.train.big.gan", train_ids_path,  vocabulary, tokenizer)

  # Create token ids for the development data.
  dev_ids_path = data_dir + ("/ids%d.valid.gan" % vocabulary_size)
  #query_dev_ids_path = data_dir + (".ids%d.query" % vocabulary_size)
  data_to_token_ids(data_dir + "/data.valid.big", dev_ids_path, vocabulary, tokenizer)
  #data_to_token_ids(dev_path + ".query", query_dev_ids_path, vocabulary, tokenizer)

  return (train_ids_path, dev_ids_path)
